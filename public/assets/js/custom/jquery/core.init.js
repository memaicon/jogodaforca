...
function junotoys_login_validate(form) {
	...
	var error = junotoys_form_validate(form, {
		...
		rules: [
			{
				field: "email",
				min_length: { value: 7, message: 'E-mail Inválido' },
				max_length: { value: 60, message: 'E-mail Inválido' },
				mask: { value: JUNOTOYS_STORAGE['email_mask'], message: 'E-mail Inválido' }
			},
			{
				field: "password",
				min_length: { value: 6, message: 'Minímo 6 caracteres' },
				max_length: { value: 30, message: 'Máximo 30 caracteres' }
			},
		]
	});
	if (!error) {
		jQuery.ajax({
			...
			if (rez.error === '') {
				result_box.addClass('sc_infobox sc_infobox_style_success').html('Logado com sucesso!');
				setTimeout(function() { 
					location.reload(); 
					}, 3000);
			} else {
				result_box.addClass('sc_infobox sc_infobox_style_error').html('Erro ao logar.' + '<br>' + rez.error);
			}
...
function junotoys_registration_validate(form) {
	...
	var error = junotoys_form_validate(form, {
		...
		rules: [
			{
				field: "name",
				min_length: { value: 3, message: 'Minímo 3 caracteres' },
				max_length: { value: 60, message: 'Máximo 60 caracteres' }
			},
			{
				field: "email",
				min_length: { value: 7, message: 'E-mail Inválido' },
			...
			},
			{
				...
				equal_to: { value: 'password', message: 'Confirmação de senha incorreta' }
			}
		]
	});
	if (!error) {
		jQuery.ajax({
			...
			data: {
				name: jQuery('#registration_username').val(),
				email: jQuery('#registration_email').val(),
				password: jQuery('#registration_pwd').val(),
				password_confirmation: jQuery('#registration_pwd2').val()
			},
			dataType: 'json',
		}).done(function(response) {
			var rez = {};
			...
			result_box.toggleClass('sc_infobox_style_error', false).toggleClass('sc_infobox_style_success', false);
			if (rez.error === '') {
				result_box.addClass('sc_infobox sc_infobox_style_success').html('Registrado com sucesso!');
				setTimeout(function() { 
					jQuery('.popup_login_link').trigger('click'); 
				}, 3000);
			} else {
				result_box.addClass('sc_infobox sc_infobox_style_error').html('Falha ao registrar' + ' ' + rez.error);
			}
			result_box.fadeIn().delay(3000).fadeOut();
		});
	}
