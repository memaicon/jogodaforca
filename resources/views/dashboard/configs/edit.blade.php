    ...
    <div class="col-sm-12">
        <div class="card-box">
            ...
            <div class="box-body">
              ...
              <div class="form-group{{ $errors->has('type_id') ? ' has-error' : '' }}">
                ...
                <select class="form-control" id="type_id" name="type_id" placeholder="Tipo">
                  @foreach(\App\Models\Config\Type::all() as $item)
                      <option value="{{ $item->id }}" {{ $config->type->id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                  @endforeach
                </select>
              ...
              @elseif($config->type->id == 4)

              @elseif($config->type->id == 5)
                <div class="form-group">
                  <label for="value">Valor</label>
